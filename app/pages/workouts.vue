<template>
  <div class="text-white py-6 px-0 sm:px-6 max-w-64 sm:max-w-2xl mx-auto">
    ...
    <div v-else-if="store.user">
      <div v-if="workouts.length">
        <ul class="space-y-4">
          <li
            ...
            :key="workout.id"
            class="bg-[#1e1e1e] p-4 rounded-xl border border-white/10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:border-white/30"
          >
            <div>
      ...
      <div v-else class="h-32 bg-[#1e1e1e] p-4 rounded-xl border border-white/10 text-sm flex items-center justify-center text-center">
        No saved workouts yet. Create one from + New Workout.
      </div>
    </div>

    <div v-else class="h-32 bg-[#1e1e1e] p-4 rounded-xl border border-white/10 text-sm flex items-center justify-center text-center">
      <p class="text-sm text-[#a2a9b0]">
